 ...
 * 个人中心首页 幂聊首页 全球生活汇 回收站
 */
public class MPSystemGcFragment extends BaseFragment implements View.OnClickListener {
    ...
    private MPSystemGcMyArticle mpSystemGcMyArticle;//我的文章
    private MPSystemGcMyPicture mpSystemGcMyPicture;//我的图片

    private TextView myarticle , mypicture;

    @Override
    protected View initView(LayoutInflater inflater, ViewGroup container) {
        view = inflater.inflate(R.layout.mp_system_gv_fragment , container , false);

        fm = getChildFragmentManager();
        init();
        initShowFragment(0);

        myarticle = (TextView) view.findViewById(R.id.mp_system_gv_fragment_myarticle);
        mypicture = (TextView) view.findViewById(R.id.mp_system_gv_fragment_mypicture);
    ...
    public void onClick(View view) {
        switch (view.getId()){
            case R.id.mp_system_gv_fragment_myarticle:
                initShowFragment(0);
                break;
            case R.id.mp_system_gv_fragment_mypicture:
                initShowFragment(1);
                break;
        }
    }
